...
int(timer_get_conf)(uint8_t timer, uint8_t *st)
{
  if (st == NULL || timer > 2 || timer < 0)
    return 1;                                                           // validação de input
  uint8_t RBC = (TIMER_RB_CMD | TIMER_RB_COUNT_ | TIMER_RB_SEL(timer)); // construção do readback command
  if (sys_outb(TIMER_CTRL, RBC) != 0)
    return 1; // avisar o i8254 que vamos ler a configuração
  if (util_sys_inb(TIMER_0 + timer, st))
    return 1; // lemos a configuração diretamente do registo associado ao timer
  return 0;
}
